...
function renderLocalFeedback() {
    ...
    // clear previous button if any (re-rendering)
    const existingBtn = document.getElementById('dl-feedback-wrapper');
    if (existingBtn) existingBtn.remove();

    const btnWrapper = document.createElement('div');
    btnWrapper.id = 'dl-feedback-wrapper';
    btnWrapper.style.textAlign = 'right';
    btnWrapper.innerHTML = `<button id="dl-feedback-btn" class="btn" style="padding: 5px 15px; font-size: 0.9rem;">Download Archive</button>`;
    feedbackList.parentNode.insertBefore(btnWrapper, feedbackList.nextSibling); // Insert after list

    document.getElementById('dl-feedback-btn').addEventListener('click', downloadLocalFeedback);
}

function downloadLocalFeedback() {
    const saved = JSON.parse(localStorage.getItem('localFeedback')) || [];
    if (saved.length === 0) return alert("No feedback to save!");

    let text = "FEEDBACK ARCHIVE\n================\n\n";
    saved.forEach(f => {
        text += `Date: ${f.date}\nEmail: ${f.email}\nMobile: ${f.mobile}\nMessage: ${f.message}\n----------------\n\n`;
    });
    ...
    a.href = url;
    a.download = 'feedback_archive.txt';
